...
import { getDb, query, run, exec } from '../src/lib/db.js';
...
async function seed() {
  ...
  let sellerAUserId;
  const existingUserA = db.prepare('SELECT id FROM users WHERE email = ?').get(sellerAEmail);
  if (!existingUserA) {
  ...
  let sellerAId;
  const existingSellerA = db.prepare('SELECT id FROM sellers WHERE slug = ?').get('luex-healthcare-ghana');
  if (!existingSellerA) {
    const res = db.prepare(`
  ...
  let sellerBUserId;
  const existingUserB = db.prepare('SELECT id FROM users WHERE email = ?').get(sellerBEmail);
  if (!existingUserB) {
  ...
  let sellerBId;
  const existingSellerB = db.prepare('SELECT id FROM sellers WHERE slug = ?').get('et-leather-collections');
  if (!existingSellerB) {
    const res = db.prepare(`
      INSERT INTO sellers (user_id, business_name, slug, whatsapp_number, phone, email, region, city, address, bio, status, delivery_notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'approved', ?)
